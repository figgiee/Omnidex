...
I propose to build a utility desktop application, named "Omnidex," designed to streamline the management of my large Unreal Engine asset collection. The app will have a fast Rust backend and a user-friendly interface built with Vue.js, using Tauri to create a lightweight and efficient program with native webviews.
The application will allow me to:
Index Local Assets: Rapidly scan specified local directories recursively, and storing it in a local SQLite database for filtering/sorting.
Scrape Marketplace Data: Query the FAB Marketplace online (or a similar reference like orbital-market.com) to retrieve detailed metadata for marketplace assets.Try and find out how the API works for FAB / Orbital Market
Match and Present: Implement a robust matching algorithm to compare my indexed local assets with the scraped marketplace data, identifying potential matches, new versions, or updates. The application will present these matches (or unmatched assets) in a user-friendly GUI.
Export/Import: Allow users to export and import their asset catalog in common formats like JSON or CSV.
...
serde, serde_json, csv: For data serialization/deserialization (JSON, CSV).
...
headless_chrome: (Conditional) If dynamic JavaScript rendering is a significant barrier for marketplace scraping.
